...
function hideAndAppened() {
	...
	//making a true arrey of the list
	let prepPage = Array.from(studentListDOM);
	for (let i=0; i<=Math.floor(studentListDOM.length/10);i++){
	

	//changing arrey every loop		
	let page = prepPage.splice(0, 10);

	//creat a button elemente
	let button = document.createElement("button");
	button.name=i;
	console.log(button);
	page.innerHTML  = page.innerHTML  + button;	

	//attach the hidden students arrey instence to the button(say that 5 times xD)
	// better yet. atttach a show and and hide trigger on click. 
		// show should trigger on click
		// hide should trigger when another button is clicked
	}
};
